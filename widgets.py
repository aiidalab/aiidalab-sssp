...
class LogOutputWidget(ipw.VBox):
    ...
    def __init__(self, num_min_lines=10, **kwargs):
        ...
        super().__init__(children=[
            ...
            ipw.HBox(children=[self._filename, self._download_link],
    ...
    def _refresh_output(self, _=None):
        ...
        with self.hold_trait_notifications():
            ...
            html_download = f'<a download="{filename}" ' \
                            f'href="data:text/plain;base64,{payload}" target="_blank">Download</a>'
    ...
    def _format_output(self, text):
        ...
        text = '\n'.join(lines)
        return f'<pre style="background-color: #1f1f2e; ' \
               f'color: white; line-height: 100%">{text}</pre>'


class ProcessOutputFollower(ipw.VBox):
    """Process output follower"""
    ...
    def __init__(self, **kwargs):
        ...
        self.selector = ipw.Dropdown(description='Calcjob:',
    ...
    def _update_calcjobs(self, process):
        ...
        with self.hold_trait_notifications():
            previous_calcjobs = {
                pk
                for _, pk in self.selector.options if pk is not None
            }
    ...
    def _start_push_threads(self):
        ...
        for pk, calcjob in self._calcjobs.items():
            if pk not in self._push_threads:
                push_thread = Thread(target=self._push_output,
                                     args=(calcjob, ))
    ...
    def _update(self, change=None):
        ...
        with self.hold_trait_notifications():
            ...
            if pk is self._SHOW_MOST_RECENTLY_UPDATED:
                calcjob = self._calcjobs.get(
                    self._most_recently_updated_calcjob)

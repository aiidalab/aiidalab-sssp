...
from aiidalab_widgets_base.viewers import DictViewer, StructureDataViewer, FolderDataViewer, BandsDataViewer
...
def viewer(obj, downloadable=True, **kwargs):
    ...
    if not isinstance(obj, Node):  # only working with AiiDA nodes
        warnings.warn(
            "This viewer works only with AiiDA objects, got {}".format(
                type(obj)))
        return obj
    ...
    def __init__(self, xydata, **kwargs):
        ...
        with out:
            ...
            for y in xydata.get_y():
                ...
                y_unit = y[2]
                # Create the figure
                plot = figure(plot_width=self._PLOT_WIDTH,
